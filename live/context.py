...
def game_context(g, state, now=None):
    ...
    return {
        ...
        "versions": [{"id": v["version_id"], "at": v["created_at"], "pHome": v.get("p_home"), "pModel": v.get("p_model"), "pMarket": v.get("p_market"),
                      "margin": v.get("margin"),
                      "homeScore": v.get("home_score"), "awayScore": v.get("away_score"), "reason": v.get("reason"),
                      "source": v.get("source"), "trigger": v.get("trigger"), "method": v.get("method"), "dataCutoff": v.get("data_cutoff"),
