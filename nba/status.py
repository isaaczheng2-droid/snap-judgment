...
SOURCES = [
    ...
    {"id": "stats_nba", "dataset": "Official box scores, rosters, player index, advanced/tracking tables", "provider": "stats.nba.com via nba_api (community client, MIT; not an officially supported API)",
     ...
     "gaps": "blocks datacenter traffic: the 2026-09-23 Actions-runner probe timed out on every endpoint (curl and nba_api); unavailable from the build container too",
     "reachable_from": "browser: yes; container: no; runner: no (timed out 2026-09-23)", "role": "none in production; adapter kept as a fallback", "verified_on": "2026-09-23 (runner probe)"},
    {"id": "espn_site", "dataset": "Current rosters (with status and injury flags) and league injury feed", "provider": "ESPN site API (site.api.espn.com), same athlete/team ids as the box scores",
     "earliest_verified_season": "current only (no history)", "update_frequency": "live; injury entries timestamped", "subscription": "none",
     "gaps": "unofficial and undocumented; not the official NBA injury report (which is used when published); no minutes restrictions", "reachable_from": "browser: yes; runner: yes (same host the NFL page already uses)",
     "role": "current rosters, availability feed, snapshot archive", "verified_on": "2026-09-23"},
    ...
    {"id": "pbpstats", "dataset": "Possession totals, lineups, on/off", "provider": "api.pbpstats.com", "earliest_verified_season": "2000-01 (site); 2024-25 team totals verified",
     ...
     "reachable_from": "browser: yes; runner: yes (200 on totals and game logs, 2026-09-23); container: no", "role": "lineup units, on/off, possession-level pace (needed before defence is graded)", "verified_on": "2026-09-23 (runner probe)"},
    ...
    {"id": "official_injury", "dataset": "Official NBA injury report (PDF, several times daily in season)", "provider": "official.nba.com / ak-static.cms.nba.com",
     ...
     "gaps": "PDF parsing; no report until the season starts (Oct 2026); historical snapshots begin with forward collection", "reachable_from": "runner: index page yes (200); PDFs verified once published", "role": "availability, P(play) inputs (wins over the ESPN feed when both exist)", "verified_on": "2026-09-23 (runner probe)"},
